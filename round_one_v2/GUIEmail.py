...
class Email:
    ...
    def __init__(self):
        ...
        _init_toolbar(self)
        self._init_gridbox()
        self.frm_insert_email = None
        self.frm_edit_email = None
        self.add_email_flag = False

    def _init_gridbox(self):
        self.mlb = MultiListbox(self.frame, (('UID', 3), ('Email', 25), ('Label', 25)))
       # tb_email = sql.session._query("select e.unique_id, e.email, e.label from people p, email e where p.unique_id = e.unique_id")
        tb_email = sql.session._query("select * from email")
        self.update_mlb(tb_email)
        self.mlb.pack(expand=YES, fill=BOTH)

    # insert_email button clicked()
    def btn_add_click(self):
        if self.add_email_flag:
            return 0
        self.add_email_flag = True
        self.frm_insert_email = AddEmail()
        self.frame.wait_window(self.frm_insert_email.frame)
        if self.frm_insert_email._okbtn_clicked == 1:
            tbEmail = sql.session._query("select * from email ")
            self.update_mlb(tbEmail)
        self.add_email_flag = False
    ...
    def btn_del_click(self):
        ...
        sql.session.delete_email(int(self.mlb.item_selected[1]))
    ...
    def btn_search_click(self):
        ...
        tb_email = sql.session.search_email(fnd)
        self.update_mlb(tb_email)
...
class AddEmail:
    ...
    def _init_widgets(self):
        self.label1 = Label(self.frame, text="Email")
    ...
    def btnok_click(self):
        ...
        if '' in items:
            ...
            return 'break'
        sql.session.insert_email(items)
